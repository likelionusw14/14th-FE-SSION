...
function render() {
  ...
  visibleMembers.forEach((member) => {
    ...
    li.textContent = `${member.name} ${member.present ? '✅' : '⬜'}`;
  ...
  const prmem = members.filter((m) => m.present === true);
  totalCountEl.textContent = members.length;
  presentCountEl.textContent = prmem.length;
}
...
filterBtns.forEach((btn) => {
  btn.addEventListener('click', () => {
    ...
    //안하면 액티브가 2개 이상이 되니 2개 다 활성화 된것 처럼
